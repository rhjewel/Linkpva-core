...
if (! class_exists('CSF_Setup')) {
  class CSF_Setup
  {
    ...
    {
      load_textdomain('linkpva-core', self::$dir . '/languages/' . get_locale() . '.mo');
    }
    ...
    public static function add_admin_enqueue_scripts()
    {
      ...
      // Main style
      wp_enqueue_style('linkpva-core', self::include_plugin_url('assets/css/style' . $min . '.css'), array(), self::$version, 'all');
      ...
      wp_enqueue_script('linkpva-core', self::include_plugin_url('assets/js/main' . $min . '.js'), array('csf-plugins'), self::$version, true);

      // Main variables
      wp_localize_script('linkpva-core', 'csf_vars', array(
        'color_palette'     => apply_filters('csf_color_palette', array()),
        'i18n'              => array(
          'confirm'         => esc_html__('Are you sure?', 'linkpva-core'),
          'typing_text'     => esc_html__('Please enter %s or more characters', 'linkpva-core'),
          'searching_text'  => esc_html__('Searching...', 'linkpva-core'),
          'no_results_text' => esc_html__('No results found.', 'linkpva-core'),
        ),
      ));
    ...
    public static function field($field = array(), $value = '', $unique = '', $where = '', $parent = '')
    {
      ...
      if (! empty($field['_notice'])) {
        ...
        $field            = array();
        $field['content'] = esc_html__('Oops! Not allowed.', 'linkpva-core') . ' <strong>(' . $field_type . ')</strong>';
        $field['type']    = 'notice';
        $field['style']   = 'danger';
      }
      ...
      if (! empty($field_type)) {
        ...
        if (class_exists($classname)) {
          ...
          echo '<p>' . esc_html__('Field not found!', 'linkpva-core') . '</p>';
        }
      } else {
        echo '<p>' . esc_html__('Field not found!', 'linkpva-core') . '</p>';
      }
