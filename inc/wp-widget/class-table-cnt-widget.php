...
class Egns_Table_Cnt_Custom_Widget extends WP_Widget
{
    ...
    function __construct()
    {
        parent::__construct(
            ...
            __('Egns Table of Contents', 'linkpva-core'),
            array(
                'description' => __('Displays table of contents links with custom section targets.', 'linkpva-core'),
            )
        );
    }
    ...
    {
        $title = isset($instance['title']) ? apply_filters('widget_title', $instance['title']) : __('Table of Content', 'linkpva-core');
...
?>
    ...
    {
        $title = isset($instance['title']) ? $instance['title'] : __('Table of Content', 'linkpva-core');
        $items = $this->get_items($instance);
        $items = array_pad($items, self::MAX_ITEMS, array('label' => '', 'target' => ''));
    ?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php echo esc_html__('Title:', 'linkpva-core'); ?></label>
            <input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr($title); ?>" />
        </p>

        <p><?php echo esc_html__('Add each sidebar item label and the matching section ID from the post content, for example #section1.', 'linkpva-core'); ?></p>

        <?php for ($i = 0; $i < self::MAX_ITEMS; $i++) : ?>
            <p>
                <strong><?php echo esc_html(sprintf(__('Item %d', 'linkpva-core'), $i + 1)); ?></strong>
                <label for="<?php echo esc_attr($this->get_field_id('items_' . $i . '_label')); ?>"><?php echo esc_html__('Label:', 'linkpva-core'); ?></label>
                ...
                <label for="<?php echo esc_attr($this->get_field_id('items_' . $i . '_target')); ?>"><?php echo esc_html__('Target ID:', 'linkpva-core'); ?></label>
...
<?php
    ...
    {
        return array(
            array('label' => __('Overview of Global Tax Law.', 'linkpva-core'), 'target' => '#section1'),
            array('label' => __('Quotation of Andreson.', 'linkpva-core'), 'target' => '#section2'),
            array('label' => __('Why Cost Control Is Critical for Industrial Businesses.', 'linkpva-core'), 'target' => '#section3'),
            array('label' => __('Key Cost Control Strategies.', 'linkpva-core'), 'target' => '#section4'),
            array('label' => __('Business Benefits of Smart Cost Control.', 'linkpva-core'), 'target' => '#section5'),
            array('label' => __('Conclusion', 'linkpva-core'), 'target' => '#section6'),
        );
    }
