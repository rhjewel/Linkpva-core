...
add_action('csf_loaded', function () {
    ...
    $version    = '<small>' . sprintf(__('- Version %s', 'linkpva-core') . '</small>', $version);
    $author     = $info->get('Author');
    $author_uri = $info->get('AuthorURI');
    $author_uri = '<small>' . esc_html__('by', 'linkpva-core') . ' <a target="_blank" href="' . esc_url($author_uri) . '">' . esc_html($author) . '</a></small>';
    ...
    CSF::createOptions($prefix, array(
        ...
        'framework_title' => sprintf(__('%1$s option panel %2$s %3$s', 'linkpva-core'), $name, $version, $author_uri),
        'framework_class' => 'linkpva-core',

        /*--------------------------
            MENU SETTINGS
            ---------------------------*/
        'menu_title'      => esc_html__('Linkpva Options', 'linkpva-core'),
        ...
        'footer_credit'  => sprintf(__('Credited %s', 'linkpva-core'), $author_uri),
        /* translators: %s: author url */
        'footer_text'    => sprintf(__('Made with love %s', 'linkpva-core'), $author_uri),
        'footer_after'   => '',
        'transient_time' => 0,
